...
export function TabAssignment({ id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setListGrade(res?.data?.data?.sort((a, b) => a.position - b.position));
        setMessage("");
      } catch (error) {
  ...
  function handleDelete(e, value) {
    ...
    async function deleteGrade() {
      try {
        ...
        if (res.status === 201) {
          ...
          setListGrade(
            listGrade.filter((item) => item.assignmentId !== value.assignmentId)
          );
        }
      } catch (error) {
    ...
    if (
      window.confirm("Are you sure you want to delete this grade composition?")
    ) {
      deleteGrade();
    } else {
      return;
