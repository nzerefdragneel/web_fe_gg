...
export function TabAssignment({ id }) {
  ...
  useEffect(() => {
    ...
    listAssignments?.forEach((item, index) => {
      updatePosition(item.assignmentId, index + 1);
    });
  }, [listAssignments]);

  const onSortEnd = ({ oldIndex, newIndex }) => {
    if (listAssignments.length === 0) return;
    if (oldIndex === newIndex) {
      return;
    }
    setlistAssignments(arrayMove(listAssignments, oldIndex, newIndex));
  };
    ...
    async function updateGrade() {
      try {
        if (Number.isInteger(parseInt(scale)) && !scale?.includes(".")) {
          gradeService
            .updateNameScale(gradeName, scale, gradeUpdate.assignmentId)
            .then(
              (res) => {
                if (res.status === 201) {
